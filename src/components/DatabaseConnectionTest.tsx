...
interface ConnectionTestResult extends TestResult {
  latencyMs: number;
}

interface ReadTestResult extends TestResult {
  tableCount: number;
  tables: string[];
}

interface WriteTestResult extends TestResult {
  comment?: string;
  commentId?: number | null;
}

interface TablesTestResult extends TestResult {
            ...
            {testResults.read.tables && (
              <div className="mt-2">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-1 mt-1">
                  {testResults.read.tables.map((table, idx) => (
                    <div key={idx} className="bg-gray-100 px-2 py-1 rounded text-sm">{table}</div>
                  ))}
                </div>
              </div>
            )}
...
const TestResultItem: React.FC<TestResultItemProps> = ({ title, result, details = [], children }) => {
